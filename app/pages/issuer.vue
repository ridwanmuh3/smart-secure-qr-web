...
<template>
  <div class="max-w-3xl mx-auto px-6">
    <div class="mb-8">
      ...
      <p class="text-sm text-gray-500 mt-1">Sign your document and generate a secure QR code with time-lock encryption</p>
    </div>
    ...
    <Transition name="page" mode="out-in">
      <div v-if="result" class="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm">
        <IssuerQRCodeDisplay
          ...
          :signed-pdf-base64="result.signed_pdf_base64"
      ...
      <div v-else class="space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-sm card-hover">
          ...
          <IssuerTimeWindowConfig v-model:valid-from="validFrom" v-model:valid-until="validUntil" @preset="setTimePreset" />
        </div>
        ...
        <Transition name="page">
          <div v-if="isPDF" class="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-sm card-hover">
            <h2 class="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
              <span class="w-6 h-6 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center text-xs font-bold">4</span>
              QR Position on PDF
            </h2>
            <IssuerQRPositionConfig
              :position="qrPosition"
              :page="qrPage"
              :size="qrSize"
              :is-pdf="isPDF"
              @update:position="qrPosition = $event"
              @update:page="qrPage = $event"
              @update:size="qrSize = $event"
            />
          </div>
        </Transition>
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-sm card-hover">
          <h2 class="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
            <span class="w-6 h-6 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center text-xs font-bold">{{ isPDF ? '5' : '4' }}</span>
            Additional Information
          </h2>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1.5">Issuer ID</label>
              <input v-model="issuerID" type="text" placeholder="Optional" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all duration-200" />
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1.5">Metadata</label>
              <input v-model="metadata" type="text" placeholder="Optional" class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all duration-200" />
            </div>
          </div>
        </div>
        ...
        <button
          ...
          :disabled="isGenerating || !fileName || !selectedKeyId || !validFrom || !validUntil"
          @click="generateQR"
        >
