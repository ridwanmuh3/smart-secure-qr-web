...
<template>
  <div class="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-50/30 flex items-center justify-center p-4">
    <div class="w-full max-w-md">
      ...
      <div v-else-if="result && statusConfig" class="space-y-4">
        ...
        <!-- Scan count warning -->
        <div v-if="result.scan_count && result.scan_count > 5" class="p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-700 text-center font-medium">
          <Icon icon="lucide:alert-triangle" class="inline w-4 h-4 mr-1" />
          Scanned {{ result.scan_count }} times — beware of potential cloning
        </div>

        <!-- Details -->
        <div v-if="result.file_name" class="bg-white rounded-2xl border border-gray-200/80 shadow-sm p-5 text-sm space-y-3">
          <div v-if="result.file_name" class="flex justify-between"><span class="text-gray-500">File</span><span class="text-gray-700 font-medium">{{ result.file_name }}</span></div>
          <div v-if="result.file_size" class="flex justify-between"><span class="text-gray-500">Size</span><span class="text-gray-700">{{ formatSize(result.file_size) }}</span></div>
          ...
          <div v-if="result.valid_from" class="flex justify-between"><span class="text-gray-500">Valid From</span><span class="text-gray-700 text-xs">{{ formatDate(result.valid_from) }}</span></div>
          <div v-if="result.valid_until" class="flex justify-between"><span class="text-gray-500">Valid Until</span><span class="text-gray-700 text-xs">{{ formatDate(result.valid_until) }}</span></div>
          <div v-if="result.document_hash" class="flex justify-between"><span class="text-gray-500">Hash</span><span class="font-mono text-xs text-gray-600 truncate max-w-[160px]" :title="result.document_hash">{{ result.document_hash }}</span></div>
          <div v-if="result.scan_count" class="flex justify-between"><span class="text-gray-500">Total Scans</span><span class="text-gray-700">{{ result.scan_count }}</span></div>
        </div>
      </div>
