...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-center">
      <span
        class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-emerald-50 text-emerald-700 text-sm font-semibold border border-emerald-200">
    ...
    <div v-if="pdfUrl" class="border border-gray-200 rounded-2xl overflow-hidden bg-white shadow-sm">
      <object :data="pdfUrl" type="application/pdf" class="w-full h-125">
    ...
    <div class="flex justify-center gap-3">
      <button v-if="pdfUrl"
        class="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 active:scale-[0.98] text-sm font-semibold transition-all duration-200 shadow-sm shadow-emerald-200 cursor-pointer"
        @click="emit('savePdf')">
        <Icon icon="lucide:download" class="w-4 h-4" />
        Download Signed PDF
      </button>
      <button
        class="flex items-center gap-2 px-6 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 cursor-pointer"
        :class="pdfUrl ? 'border border-gray-200 text-gray-700 hover:bg-gray-50' : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm shadow-emerald-200'"
        @click="emit('save')">
        <Icon icon="lucide:download" class="w-4 h-4" />
        Save QR Image
      </button>
      <button
        class="flex items-center gap-2 px-6 py-2.5 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 text-sm font-medium transition-all duration-200 cursor-pointer"
        @click="emit('reset')">
        Create New
      </button>
    </div>
    ...
    <div class="bg-gray-50/80 rounded-2xl p-5 text-sm space-y-2.5 border border-gray-200/80">
      ...
      <div class="flex justify-between">
        ...
        <span class="font-mono text-xs text-gray-700 truncate max-w-50" :title="documentHash">{{ documentHash
          }}</span>
      </div>
      <div class="flex justify-between">
