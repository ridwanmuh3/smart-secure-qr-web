...
<template>
  <div>
    <div v-if="keys.length === 0" class="text-sm text-gray-500 p-4 bg-amber-50 rounded-xl border border-amber-200">
      <p>No keys yet. <NuxtLink to="/keys" class="text-emerald-600 font-semibold hover:underline">Create a key
        </NuxtLink> first.</p>
    </div>

    <div v-else-if="signingKeys.length === 0"
      class="text-sm text-amber-700 p-4 bg-amber-50 rounded-xl border border-amber-200">
    ...
    <div v-else class="flex gap-2">
      ...
      <button
        class="px-3 py-2 text-gray-500 hover:text-emerald-600 border border-gray-200 rounded-xl hover:bg-emerald-50 transition-colors cursor-pointer"
        @click="emit('refresh')">
